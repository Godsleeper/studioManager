...
class RegistrationForm extends React.Component {
    ...
    infoAdd (data) {
        axios
            .post('/teacher/personInfoAdd', {info: data})
            .then((res) => {
    ...
    infoModify (data) {
        axios
            .post('/teacher/personInfoModify', {info: data})
            .then((res) => {
                var {status} = res.data;
                switch (status) {
                    case 'NoInfo':
                        message.error('请直接添加个人信息');
                        break;
                    case 'Success':
    ...
    infoRequest () {
        axios.get('/teacher/personInfoGet')
            .then((res) => {
                if (res.data) {
                    this.setState({
            ...
            <Form onSubmit={this.handleSubmit}>
                ...
                >
                    {getFieldDecorator('position', {
                        rules: [
                            { required: true, message: '请选择你的职称' },
                        ],
                        initialValue: this.state.info.position
                    })(
                        <Select placeholder="请选择你的职称">
                            {config.teacherPosition.map((item, index) => {
                ...
                >
                    {getFieldDecorator('address',{
                        ...
                        <Input type="textarea" placeholder="实验室详细地址" autosize={{ minRows: 2, maxRows: 6 }} />
                    )}
                </FormItem>
